// 发布 v1.0.10 Release：创建 release → 上传 EXE + version.json
const fs = require('fs');
    ...
    release = await api('/repos/' + REPO + '/releases', {
      ...
      body: JSON.stringify({
        ...
        body: [
          '## v1.0.10 更新内容',
          '',
          '- 仪表盘按业务分类显示：默认展示账号首个业务的数据统计
- 归属多个业务的账号可在仪表盘顶部切换业务范围（单业务自动采用、不显示切换条）
- 修复 v1.0.8 启动卡在「正在加载系统…」的问题
        ].join('\n')
      })
    });
  ...
  const assets = [
    { file: path.join(root, 'GZInspection_v1.0.10.exe'), name: 'GZInspection_v1.0.10.exe', type: 'application/octet-stream' },
