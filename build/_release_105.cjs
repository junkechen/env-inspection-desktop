// 发布 v1.0.5 Release：创建 release → 上传 EXE + version.json
const fs = require('fs');
...
(async () => {
    ...
    release = await api('/repos/' + REPO + '/releases', {
      ...
      body: JSON.stringify({
        ...
        body: [
          '## v1.0.5 更新内容',
          '',
          '- 用户管理：操作列改为「操作 ▾」下拉菜单，彻底修复「删除」按钮被固定列裁掉看不见的问题',
          '- 下拉菜单包含：编辑 / 重置密码 / 通过审核 / 禁用或启用 / 删除',
          '- 含 v1.0.4 的「业务类型」多选分类（安全业务 / 能源环保业务 / 现场业务 / 设备业务）'
        ].join('\n')
      })
    });
  ...
  const assets = [
    { file: path.join(root, 'GZInspection_v1.0.5.exe'), name: 'GZInspection_v1.0.5.exe', type: 'application/octet-stream' },
