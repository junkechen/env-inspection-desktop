// 发布 v1.0.11 Release：创建 release → 上传 EXE + version.json
const fs = require('fs');
...
(async () => {
    ...
    release = await api('/repos/' + REPO + '/releases', {
      ...
      body: JSON.stringify({
        ...
        body: [
          '## v1.0.11 更新内容',
          '',
          '- 仪表盘统计口径与手机端对齐：按业务统计全部隐患（原仅统计与自己相关，导致部分账号计数为0）',
          '- 归属多个业务的账号可在仪表盘顶部切换业务范围（单业务自动采用）'
        ].join('\n')
      })
    });
  ...
  const assets = [
    { file: path.join(root, 'GZInspection_v1.0.11.exe'), name: 'GZInspection_v1.0.11.exe', type: 'application/octet-stream' },
